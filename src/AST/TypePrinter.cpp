...
// TODO(FiTH)
void my_printTemplateArgumentList(raw_ostream &OS,
...
void TypePrinter::printTemplateId(const TemplateSpecializationType *T,
  ...
  my_printTemplateArgumentList(OS, T->template_arguments(), Policy);
  spaceBeforePlaceHolder(OS);
}
...
static void printArgument(const TemplateArgument &A, const PrintingPolicy &PP,
  ...
  if (A.getKind() == TemplateArgument::ArgKind::Type) {
    PrintingPolicy SubPolicy(PP);
    SubPolicy.SuppressStrongLifetime = true;
    // getAsType().print(Out, SubPolicy);

    // TODO(FiTH)
    SubPolicy.SuppressUnwrittenScope = false;
...
static void printTo(raw_ostream &OS, ArrayRef<TA> Args,
  ...
  for (const auto &Arg : Args) {
    ...
    if (Argument.getKind() == TemplateArgument::Pack) {
      ...
      printArgument(
          Arg, Policy, ArgOS,
          false); // TODO(FiTH): TemplateParameterList::shouldIncludeTypeForArgument(TPL, ParmIndex));
    }
...
void my_printTemplateArgumentList(raw_ostream &OS,
  ...
  my_printTemplateArgumentList(OS, Args.arguments(), Policy, TPL);
}
...
// TODO(FiTH)
std::string transformer::AST::GetQualTypeAsString(const clang::QualType& qual_type, const clang::PrintingPolicy &Policy)
{
  const auto split = splitAccordingToPolicy(qual_type, Policy);
